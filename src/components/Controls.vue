<template>
	<div class="controls">
		...
		<div v-for="(controls, name) in settings" :key="name" class="section">
			...
			<div v-for="(control, n) in controls" :key="n" class="control" :class="{row: control.type === 'select'}">
...
<style scoped>
 .controls { min-width: 400px; padding: 20px; display: inline-block; user-select: none; background-color: var(--overlay-color); cursor: auto; }
 	h2, h3 { margin-top: 0; }
